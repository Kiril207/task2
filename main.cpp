...
int main() {
    ...
    RenderWindow window(VideoMode(Vector2u(w, h)), "Snake Game!");
    ...
    while (window.isOpen()) {
        ...
        for (int i=0; i<N; i++)
            for (int j=0; j<M; j++) {
                sprite1.setPosition(Vector2f(i * size, j * size));
                window.draw(sprite1);
            }

        for (int i=0; i<num; i++) {
            sprite2.setPosition(Vector2f(s[i].x * size, s[i].y * size));
            window.draw(sprite2);
        }

        sprite2.setPosition(Vector2f(f.x * size, f.y * size));
        window.draw(sprite2);
